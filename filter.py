...
est = np.zeros((50,3))
t = (1-p)/p
def filter_pdf(s1,s2,p,m,est):
    state = filter(t,s1,s2,p,est)
...
for idx in range(50):
    ...
    for m in range(len(M)):
        for data in range(1000):
            dataset[m][data] = filter_pdf(s1+np.array(s1_var)[data],s2+np.array(s2_var)[data],np.array(b)[data][0],M[m],estimation[0])
